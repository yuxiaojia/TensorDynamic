...
// Profile HMMA execution - collect actual thread indices for first N executions
extern "C" __device__ __noinline__ void profile_hmma(
    int pred,
    int kernel_id,
    uint64_t pprofile,
    uint64_t ptarget_list,
    int max_threads_to_record,
    uint64_t pstart_flag
) {
    bool* start_flag = (bool*)pstart_flag;
    ...
    atomicAdd(&profile->total_executions, 1);

        // Try to record this thread's indices if we haven't hit the limit
    uint32_t slot = atomicAdd(&profile->num_recorded_threads, 1);

    if (slot < max_threads_to_record) {
                // Successfully got a slot - record this thread's execution
        profile->executed_threads[slot].block_x = blockIdx.x;
        profile->executed_threads[slot].block_y = blockIdx.y;
        profile->executed_threads[slot].block_z = blockIdx.z;
        profile->executed_threads[slot].thread_x = threadIdx.x;
        profile->executed_threads[slot].thread_y = threadIdx.y;
        profile->executed_threads[slot].thread_z = threadIdx.z;

        // Also add to injection target list for immediate use
        int target_slot = atomicAdd(&target_list->num_targets, 1);
        if (target_slot < MAX_TARGETS) {
            target_list->targets[target_slot].target_block_x = blockIdx.x;
            target_list->targets[target_slot].target_block_y = blockIdx.y;
            target_list->targets[target_slot].target_block_z = blockIdx.z;
            target_list->targets[target_slot].target_thread_x = threadIdx.x;
            target_list->targets[target_slot].target_thread_y = threadIdx.y;
            target_list->targets[target_slot].target_thread_z = threadIdx.z;
            target_list->targets[target_slot].target_instr = 0; // 0 means any instruction
        }
    } else if (slot == max_threads_to_record) {
                // This is the thread that filled the last slot - disable further profiling
        *start_flag = false;
    }
}
...
extern "C" __device__ __noinline__ void hmma_replace(
    ...
    for (int i = 0; i < target_list->num_targets; i++) {
        ...
        if (blockIdx.x == target->target_block_x &&
            ...
            (target->target_instr == 0 || instr_idx == target->target_instr)) {
            is_target = true;
            break;
        }
    }
    ...
    if (loc_idx < MAX_INJECTED_TRACKING) {
        ...
        // Read the current destination register value (32 bits)
    int raw_bits = nvbit_read_reg(reg_dst_num);

        // Extract FP16 values for recording
    ...
    *reinterpret_cast<unsigned short*>(&original_half_high) = (unsigned short)((raw_bits >> 16) & 0xFFFF);
    ...
    if (injection_mode == 0) {
        ...
        // Write the corrupted value back to register
